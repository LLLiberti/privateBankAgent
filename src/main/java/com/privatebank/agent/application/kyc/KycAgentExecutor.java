...
public class KycAgentExecutor implements BusinessAgentExecutor<KycMaskedInput, KycStructuredResult> {
    ...
    public static final String SYSTEM_PROMPT = """
            ...
            当 MGR-* 与 SRC-* 冲突时，不得静默覆盖已有事实，应在 dataGaps 或 riskAlerts 中说明冲突并建议人工复核。MGR-* 不属于 Neo4j 图谱证据，不得用于 graphAssessment.evidenceRefs。
            ...
            只能引用输入中已有的 SRC-* 或 MGR-* 编号，不得编造证据编号，不得输出原始姓名、企业名、联系方式、地址或其他被禁止信息。
            ...
            findings、riskAlerts、recommendedActions、dataGaps 各自最多 20 项；每条 finding 的 evidenceRefs 最多 10 项，且每项必须是允许引用列表中的单个 SRC-* 或 MGR-* 编号。
    ...
    private String userPrompt(KycMaskedInput input, KycOutputValidationException validationFailure) {
        String instruction = validationFailure == null
                ...
                : "上一版结构化结果未通过证据、脱敏或业务约束校验。具体失败原因："
                        ...
                        + "。允许引用的全部证据编号："
                        + input.allowedEvidenceRefs().stream().sorted().toList()
